...
const LoginForm = ({
  ...
  const { user, login, regist } = useAuth();
  ...
  const handleRegist = ()  => {
    regist(usernameRef?.current?.value!, emailRef?.current?.value!, passwordRef?.current?.value!)
  }
      ...
      >
        ...
        <div className="w-11/12 flex flex-col items-center gap-10 mt-6 justify-center ">
          ...
          {isLogin ? (
            ...
            <button className="flex items-center gap-4 justify-center bg-blue-600 text-white px-2 py-4 w-full rounded-md mt-3 font-bold"
