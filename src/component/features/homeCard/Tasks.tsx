...
const Tasks = ({ card }: { card: PostType }) => {
  ...
  const nextTask = card.taskFlow.find((task) => task.next === true);

  return (
    <div className="flex justify-around items-center mt-4">
      <div className="flex flex-col items-center text-center w-4/12 my-2">
        <h2 className="">現在のタスク</h2>
        <p className="rounded-full w-5 h-5 bg-green-500 mr-2 animate-pulse ml-2 mt-5 mb-3"></p>
        <p className="font-bold">{currentTask?.task}</p>
      </div>
      <div className="w-2/12 text-center text-green-500">
      ...
      <div className="flex flex-col items-center text-center w-4/12">
        ...
        <p className="font-bold mt-1">{nextTask?.task}</p>
      </div>
    </div>
    // <div>
